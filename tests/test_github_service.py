from gistapi.github_service import (
    parse_total_number_of_pages,
    gist_has_pattern,
    fetch_all_gist_contents,
    fetch_and_search,
)
...
def test_parse_total_number_of_pages():
    link = """
        <https://api.github.com/user/abcd/gists?page=2>; rel="next", <https://api.github.com/user/abcd/gists?page=10>; rel="last"
    """
    assert parse_total_number_of_pages(link=link) == 10


def test_gist_has_pattern_no_pattern():
    gist = {"files": {"file1.py": {"content": "import abcd \n print('hello world')"}}}
    pattern = "import lmno"
    assert gist_has_pattern(gist, pattern) == False


def test_gist_has_pattern():
    gist = {"files": {"file1.py": {"content": "import lmno \n print('hello world')"}}}
    pattern = "import lmno"
    assert gist_has_pattern(gist, pattern) is True
...
def test_fetch_all_gist_contents(mocker):
    ...
    gists_api_mock = mocker.patch(
        "gistapi.github_service.fetch_gists_for_username",
        return_value=(gist_list, None),
    )
    details_api_mock = mocker.patch(
        "gistapi.github_service.fetch_gist_details", return_value=gist_details
    )
    results = fetch_all_gist_contents(username="testuser")

    assert results == [gist_details]
...
def test_fetch_and_search_no_matches(mocker):
    ...
    assert (
        result.get("info")
        == "This user does not have any code with this pattern present"
    )
    assert result.get("matches") == []
